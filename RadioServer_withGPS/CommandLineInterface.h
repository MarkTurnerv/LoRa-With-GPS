/*****************************************************************************
https://projecthub.arduino.cc/mikefarr/simple-command-line-interface-fbb961
...
*/
const char *sleepCommandToken       = "sleep";                     //Modify here
const char *setBWCommandToken  = "setBW";                     //Modify here
const char *setSFCommandToken  = "setSF";                     //Modify here
const char *setCRCommandToken  = "setCR";                     //Modify here
...
void sleepCommand() {                                      //puts GPS-transmitting LoRa to sleep
...
void setBWCommand() {                                //7800-250000
  int bandwidth = readNumber();
  char cmdMsg[30];
  snprintf(cmdMsg,30, "Cmd: setBW%d", bandwidth);
  sendLen = strlen(cmdMsg);
...
void setSFCommand() {                                //valid inputs: integer 6-12
  int bandwidth = readNumber();
  char cmdMsg[30];
  snprintf(cmdMsg,30, "Cmd: setSF%d", bandwidth);
  sendLen = strlen(cmdMsg);
  rf95.send((uint8_t *) cmdMsg, sendLen);
  memset(cmdMsg, 0, sendLen);
  rf95.waitPacketSent();
...
void setCRCommand() {                                //Valid denominator values are 5, 6, 7 or 8.
  int bandwidth = readNumber();
  char cmdMsg[30];
  snprintf(cmdMsg,30, "Cmd: setCR%d", bandwidth);
  sendLen = strlen(cmdMsg);
  rf95.send((uint8_t *) cmdMsg, sendLen);
  memset(cmdMsg, 0, sendLen);
  rf95.waitPacketSent();
...
Command:  ", commandLine);
  ...
  if (strcmp(ptrToCommandName,  sleepCommandToken) == 0) {                   //Modify here
    sleepCommand();
    //print2(">    The sum is = ", result);

  }
  else if (strcmp(ptrToCommandName,  setBWCommandToken) == 0){
    setBWCommand();                                       //K&R string.h  pg. 251
      //print2(">    The difference is = ", result);
    } 
  else if (strcmp(ptrToCommandName,  setSFCommandToken) == 0){
    setSFCommand();
  }
  else if (strcmp(ptrToCommandName,  setCRCommandToken) == 0){
    setCRCommand();
  }
  else {
